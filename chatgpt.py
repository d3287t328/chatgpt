...
def connect(server, port, usessl, password, ident, realname, nickname, channels):
    while True:
        try:
            ...
            if password:
                irc.send(bytes(f"PASS {password}" + "\n", "UTF-8"))
...
while True:
    ...
    if len(chunk) <= 0:
        continue
    command = chunk[1] if data.startswith(":") else chunk[0]
    if command == "PING":
        irc.send(bytes(f"PONG {chunk[1]}" + "\n", "UTF-8"))
    elif command == "ERROR":
        print("Received ERROR from server. Reconnecting...")
        time.sleep(5)
        irc = connect(server, port, usessl, password, ident, realname, nickname, channels)
    elif command in ["471", "473", "474", "475"]:
        print(
            f"Unable to join {chunk[3]}: it can be full, invite only, bot is banned or need a key."
        )
    elif command == "KICK" and chunk[3] == nickname:
        irc.send(bytes(f"JOIN {chunk[2]}" + "\n", "UTF-8"))
        print(f"Kicked from channel {chunk[2]}. Rejoining...")
    elif command == "INVITE":
        ...
        command == "PRIVMSG"
        and chunk[2].startswith("#")
        and chunk[3] == f":{nickname}:"
    ):
        channel = chunk[2].strip()
        question = data.split(f"{nickname}:")[1].strip()
        if model in ["gpt-4", "gpt-4-0314", "gpt-4-32k", "gpt-4-32k-0314", "gpt-3.5-turbo", "gpt-3.5-turbo-0301"]:
            try:
                ...
                for answer in answers:
                    while len(answer) > 0:
                        if len(answer) <= 392:
                            irc.send(bytes(f"PRIVMSG {channel} :{answer}" + "\n", "UTF-8"))
                            answer = ""
                        else:
                            ...
                            irc.send(
                                bytes(
                                    f"PRIVMSG {channel} :{answer[:last_space_index]}"
                                    + "\n",
                                    "UTF-8",
                                )
                            )
                ...
                irc.send(
                    bytes(
                        f"PRIVMSG {channel}"
                        + " :API call timed out. Try again later.\n",
                        "UTF-8",
                    )
                )
            ...
            try:
                response = openai.Image.create(
                    prompt=f"Q: {question}" + "\nA:", n=1, size="1024x1024"
                )
                long_url = response.data[0].url
                type_tiny = pyshorteners.Shortener()
                short_url = type_tiny.tinyurl.short(long_url)
                irc.send(bytes(f"PRIVMSG {channel} :{short_url}" + "\n", "UTF-8"))
            except Exception as e:
                print(f"Error: {str(e)}")
                irc.send(
                    bytes(
            ...
            print("Invalid model.")
            irc.send(bytes(f"PRIVMSG {channel}" + " :Invalid model.\n", "UTF-8"))
            continue
    time.sleep(1)
